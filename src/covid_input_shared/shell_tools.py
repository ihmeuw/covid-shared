...
def wget(url: str, output_path: Union[str, Path]):
    ...
    """
    subprocess.run(shlex.split(f'wget -O {output_path} {url}'))
...
def unzip_and_delete_archive(archive_path: Union[str, Path], output_path: Union[str, Path]):
    ...
    """
    subprocess.run(shlex.split(f'unzip {archive_path} -d {output_path}'))
    subprocess.run(shlex.split(f'rm {archive_path}'))
